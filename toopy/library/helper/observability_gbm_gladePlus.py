...
class merged_def2():
    ...
    def doit(observatory, crossmatched_cat, zenith, moon_sep, hdul1, time_resolution, outdir):
        ...
        for i in range(0,len(crossmatched_cat)):
            coords = SkyCoord(ra=crossmatched_cat['RAJ2000'].values[i]*u.deg, 
                              dec=crossmatched_cat['DEJ2000'].values[i]*u.deg)
            target=FixedTarget(coord=coords, name=str(crossmatched_cat['HyperLEDA'].values[i]))
        ...
        constraints = [AtNightConstraint.twilight_astronomical(),
                       ...
                       AltitudeConstraint(min=alt_const*u.deg)] # ZA : 0-50 degrees
                        #zenith from alt
                        #zenith=90*u.deg-altitude
        # Define range of times to observe between
        start_time_object = Time(hdul1[0].header['DATE-OBS'], format='isot', scale='utc')
        ...
        #plt.title(constraints[0].__class__.__name__ , fontsize=20)
        outname = 'GladePlus_Observability_TimeGrid_@'+str(observatory)+'.pdf'
        fullname = os.path.join(outdir, outname)    
        plt.savefig(fullname)
        #plt.savefig(f'./obsi_ID_'+str(EVENTID)+' & Date_'+str(START)+'.pdf')
        #plt.show()
        ################################################################
        ################################################################
        fig, ax = plt.subplots(figsize=(10,5))
        for i, target in enumerate(targets):
            ax, airmass, timetoplot, altitude, zenith=merged_def2.plot_airmass_n_altitude(target, loc_observatory, time_grid, brightness_shading=True, altitude_yaxis=True)
            outname = 'GladePlus_Observability_Atmosphere_@'+str(observatory)+'.pdf'
            fullname = os.path.join(outdir, outname) 
            plt.savefig(fullname)
        ...
        #for i, target in enumerate(targets):
         #   #fig, ax = plt.subplots(figsize=(10,5))
            #ax, airmass, timetoplot, altitude, zenith=merged_def2.plot_airmass_n_altitude(target, loc_observatory, time_grid, brightness_shading=True, altitude_yaxis=True)
         #   #plt.savefig(f'./obsi2_ID_'+str(EVENTID)+' & Date_'+str(START)+'.pdf')
         #   #plt.show()
        return ax, airmass, timetoplot, altitude, zenith, c_fin, time_grid