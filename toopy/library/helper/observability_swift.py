...
class merged_def2():
    ...
    def doit(observatory, crossmatched_cat_glade2, zenith, moon_sep, night, too_span, time_resolution, outdir):
        ...
        targets=[]
        for i in range(0,len(crossmatched_cat_glade2)):
            coords = SkyCoord(ra=crossmatched_cat_glade2['RA'].values[i]*u.deg, 
                              dec=crossmatched_cat_glade2['DEC'].values[i]*u.deg)
            target=FixedTarget(coord=coords, name=str(crossmatched_cat_glade2['HyperLEDA'].values[i]))
        ...
        extent = [-0.5, -0.5+len(time_grid), -0.5, -0.5+len(crossmatched_cat_glade2)]
        c_fin= c1 & c2  & c3
